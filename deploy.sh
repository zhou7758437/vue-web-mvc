...
APP_HOME=/home/admin/vue-web-mvc  # 从package.tgz中解压出来的jar包放到这个目录下
WAR_NAME=vue-web-mvc-web-release.jar # jar包的名字
...
start_application() {
    stop
    rm -f ${APP_LOG}
    nohup  java -jar ${APP_HOME}/${WAR_NAME} >${APP_LOG}  &
    echo 'starting the spring boot app'
    exptime=0
    while true
    do
          ret=`fgrep "Started AppWebMain in" ${APP_LOG}`
          if [ -z "$ret" ]; then
              sleep 1
              ((exptime++))
...
stop_application() {
    ...
    PID=`ps ax | grep 'vue-web-mvc-web-release' | grep -v grep | awk '{print $1}'`
    echo ${PID}
    if [[ ! -z "$PID" ]]; then
    kill -15 $PID
    else
